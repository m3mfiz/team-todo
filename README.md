...
## Общие задачи: поимённое выполнение

Задача «Всем» выполняется каждым сотрудником отдельно: тап по чекбоксу
отмечает **личное** выполнение (задача уходит в Журнал этого сотрудника),
глобальный статус остаётся «open». Когда отметились **все** действующие
сотрудники — задача автоматически завершается и попадает в Журнал
администратора. В раскрытой задаче блок «Выполнение» показывает список
сотрудников с зелёными галочками и датами; админ дополнительно видит чип
прогресса «N из M» в строке задачи. Админ может закрыть общую задачу
принудительно; снятие отметки сотрудником переоткрывает задачу (отметки
остальных сохраняются). При удалении сотрудника полнота пересчитывается.
